...
from dataset_anime import get_anime_dataloader, AnimeFacesDataset
...
def get_dataloader_ddp(config, rank, world_size):
    ...
    if config['dataset']['name'] == 'anime':
        # For anime dataset, we need to handle the download differently
        from dataset_anime import download_anime_faces
        data_dir = config['dataset']['data_dir']
        os.makedirs(data_dir, exist_ok=True)
        anime_dir = download_anime_faces(data_dir)
        dataset = AnimeFacesDataset(root_dir=anime_dir, transform=transform)
    else:
    ...
    for epoch in range(config['training']['num_epochs']):
        ...
        if rank == 0:
            ...
            if (epoch + 1) % config['training']['save_interval'] == 0:
                save_checkpoint(
                    generator.module, discriminator.module, g_optimizer, d_optimizer,
